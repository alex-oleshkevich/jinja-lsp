...
use jinja_lsp::features::document_highlight::{HighlightKind, document_highlight};
...
fn hl01_loop_variable_highlights_write_and_reads() {
    ...
    assert!(
        !highlights.is_empty(),
        "must find highlights for loop variable"
    );
...
fn hl02_imported_macro_no_definition_in_file() {
    ...
    assert_eq!(
        write_count, 0,
        "no Write because definition is in another file"
    );
}
...
fn hl_9cfn_write_highlight_col_matches_byte_offset_after_multibyte() {
    ...
    assert!(
        !highlights.is_empty(),
        "must find highlights for x after multibyte prefix"
    );
    ...
    if let (Some(w), Some(r)) = (write_hl, read_hl) {
        ...
        assert_eq!(
            w.range.start_col, write_byte,
            "Write start_col must be byte offset, got {}",
            w.range.start_col
        );
        assert_eq!(
            r.range.start_col, read_byte,
            "Read start_col must be byte offset, got {}",
            r.range.start_col
        );
    }
}
...
fn hl04_builtin_filter_returns_empty() {
    ...
    assert!(
        results.is_empty(),
        "built-in filter must return empty (REQ-HL-04)"
    );
}
...
fn hl04_whitespace_inside_expression_returns_empty() {
    ...
    assert!(
        results.is_empty(),
        "whitespace inside expression must return empty"
    );
}
...
fn hl_nvch_block_named_block_write_span_not_on_keyword() {
    ...
    let col = 9u32;
    let results = document_highlight(src, 0, col, &idx, &reg);
    let write: Vec<_> = results
        .iter()
        .filter(|r| r.kind == HighlightKind::Write)
        .collect();
    assert!(!write.is_empty(), "must have a Write highlight");
    assert_eq!(
        write[0].range.start_col, 9,
        "Write span must be on the name at col 9, not the keyword at col 3"
    );
}
...
fn hl_eafo_var_with_set_suffix_in_name_not_write_site() {
    ...
    assert_eq!(
        write[0].range.start_col, x_set_col,
        "write span must be the set binding"
    );
}
...
fn hl_eafo_for_keyword_requires_word_boundary() {
    ...
    assert_eq!(
        write.len(),
        1,
        "for-loop target must produce exactly one Write"
    );
    // "item" appears at col 7 in "{% for item in items %}"
    let for_item_col = src.find("{% for item").unwrap() as u32 + 7;
    assert_eq!(
        write[0].range.start_col, for_item_col,
        "write span must be the for binding"
    );
}
...
fn jinja_lsp_kj7z_write_resolves_to_the_cursors_own_loop_not_the_first_same_named_loop() {
    ...
    assert_eq!(
        write.len(),
        1,
        "must find exactly one Write for the cursor's own loop"
    );
