...
fn macros_captures_macro_name() {
    ...
    assert!(
        caps.contains(&"post_url".to_owned()),
        "macro name not captured: {caps:?}"
    );
}
...
fn blocks_captures_block_name() {
    ...
    assert!(
        caps.contains(&"content".to_owned()),
        "block name not captured: {caps:?}"
    );
}
...
fn blocks_captures_required_flag() {
    ...
    assert!(
        caps.contains(&"required".to_owned()),
        "required flag not captured: {caps:?}"
    );
}
...
fn set_captures_variable_name() {
    ...
    assert!(
        caps.iter().any(|c| c.contains("my_var")),
        "variable name not captured: {caps:?}"
    );
}
...
fn for_captures_loop_variable() {
    ...
    assert!(
        caps.contains(&"item".to_owned()),
        "loop variable not captured: {caps:?}"
    );
}
...
fn extends_captures_path() {
    ...
    assert!(
        caps.iter().any(|c| c.contains("base.html")),
        "extends path not captured: {caps:?}"
    );
}
...
fn includes_captures_path() {
    ...
    assert!(
        caps.iter().any(|c| c.contains("header.html")),
        "include path not captured: {caps:?}"
    );
}
...
fn includes_captures_ignore_missing() {
    ...
    assert!(
        caps.iter()
            .any(|c| c.contains("ignore") || c.contains("missing")),
...
fn imports_captures_source_and_alias() {
    ...
    assert!(
        caps.iter().any(|c| c.contains("blog/macros.html")),
        "import source not captured: {caps:?}"
    );
    assert!(
        caps.contains(&"macros".to_owned()),
        "import alias not captured: {caps:?}"
    );
}
...
fn from_imports_captures_source() {
    let caps = captures(
        "from_imports",
        r#"{% from "blog/macros.html" import post_url %}"#,
    );
    assert!(
        caps.iter().any(|c| c.contains("blog/macros.html")),
        "from-import source not captured: {caps:?}"
    );
}
...
fn params_captures_positional_param() {
    ...
    assert!(
        caps.contains(&"name".to_owned()),
        "positional param not captured: {caps:?}"
    );
}
...
fn params_captures_keyword_param_with_default() {
    ...
    assert!(
        caps.contains(&"msg".to_owned()),
        "keyword param not captured: {caps:?}"
    );
    assert!(
        caps.iter().any(|c| c.contains("hi")),
        "param default not captured: {caps:?}"
    );
}
...
fn with_captures_variable() {
    ...
    assert!(
        caps.contains(&"x".to_owned()),
        "with variable not captured: {caps:?}"
    );
}
...
fn trans_captures_plural_variable() {
    ...
    assert!(
        caps.contains(&"count".to_owned()),
        "trans plural var not captured: {caps:?}"
    );
}
...
fn import_names_captures_names_and_aliases() {
    let caps = captures(
        "import_names",
        r#"{% from "m.html" import foo, bar as baz %}"#,
    );
    assert!(
        caps.contains(&"foo".to_owned()),
        "imported name not captured: {caps:?}"
    );
    assert!(
        caps.contains(&"baz".to_owned()),
        "import alias not captured: {caps:?}"
    );
}
...
fn set_unpacking_captures_both_names() {
    ...
    assert!(
        caps.iter().any(|c| c == "x"),
        "first unpacked name not captured: {caps:?}"
    );
    assert!(
        caps.iter().any(|c| c == "y"),
        "second unpacked name not captured: {caps:?}"
    );
}
...
fn for_unpacking_captures_both_names() {
    ...
    assert!(
        caps.iter().any(|c| c == "key"),
        "first loop name not captured: {caps:?}"
    );
    assert!(
        caps.iter().any(|c| c == "value"),
        "second loop name not captured: {caps:?}"
    );
}
...
fn caller_args_captures_caller_variable() {
    ...
    assert!(
        caps.contains(&"c".to_owned()),
        "caller var not captured: {caps:?}"
    );
}
...
fn references_captures_identifier() {
    ...
    assert!(
        caps.iter().any(|c| c == "post" || c == "title"),
        "reference not captured: {caps:?}"
    );
}
