...
use jinja_lsp::features::symbols::{SymbolKind, document_symbols};
...
fn state_chain_produces_symbols_for_block_template() {
    ...
    let index = state
        ...
        .templates
        .get("t.html")
        .expect("index in state");
    let syms = document_symbols(source, index);

    assert!(
        !syms.is_empty(),
        "document_symbols must produce symbols for a block template"
    );
...
fn state_chain_produces_symbols_for_macro_template() {
    ...
    let m = syms
        .iter()
        .find(|s| s.name == "render_card")
        .expect("macro must appear");
