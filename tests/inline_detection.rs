...
fn variable_arg_is_not_detected() {
    ...
    assert!(
        regions.is_empty(),
        "variable arg must not be detected: {regions:?}"
    );
}
...
fn region_records_host_offset() {
    ...
    assert_eq!(
        regions[0].host_offset, expected_offset,
        "host_offset must point to content start"
    );
}
...
fn d2di_suffix_match_does_not_trigger() {
    ...
    assert!(
        regions.is_empty(),
        "suffix of longer identifier must not match: {regions:?}"
    );
}
...
fn lvi1_host_col_counts_bytes() {
    ...
    assert_eq!(
        regions[0].host_col, 19,
        "host_col must be byte offset, got {}",
        regions[0].host_col
    );
}
...
fn inline_grammar_parses_expression() {
    ...
    assert_eq!(
        idx.macros.len(),
        1,
        "inline grammar must parse macro definition"
    );
...
fn server_state_populates_inline_ranges_on_update_file() {
    ...
    assert!(
        ikey.starts_with("views.py::"),
        "inline key must be views.py::<offset>"
    );
...
fn inline_diagnostic_position_translated_to_host_coords() {
    ...
    for d in &raw {
        ...
        assert!(
            hc >= host_offset as u32,
            "translated col {hc} must be >= host_col {}",
            host_offset
        );
...
fn unterminated_triple_quote_is_skipped() {
    ...
    assert!(
        regions.is_empty(),
        "unterminated triple-quoted literal must be skipped: {regions:?}"
    );
}
