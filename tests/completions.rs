...
fn cmp02_filter_context_offers_filters() {
    ...
    assert!(
        ...
        "truncate must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"upper"),
        "upper must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"lower"),
        "lower must be offered: {labels:?}"
    );
}
...
fn cmp02_expression_context_offers_variables_and_functions() {
    ...
    assert!(
        ...
        "set variable must be offered: {labels:?}"
    );
    // range() is a built-in function
    assert!(
        labels.contains(&"range"),
        "global function 'range' must be offered: {labels:?}"
    );
}
...
fn cmp02_statement_context_offers_keywords() {
    ...
    assert!(
        labels.contains(&"for"),
        "keyword 'for' must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"if"),
        "keyword 'if' must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"block"),
        "keyword 'block' must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"macro"),
        "keyword 'macro' must be offered: {labels:?}"
    );
}
...
fn cmp02_attribute_context_offers_attrs_for_known_receiver() {
    ...
    assert!(
        !items.is_empty(),
        "attribute context for 'loop' must offer items"
    );
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"index"),
        "loop.index must be offered: {labels:?}"
    );
    assert!(
        labels.contains(&"first"),
        "loop.first must be offered: {labels:?}"
    );
}
...
fn cmp03_unknown_receiver_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "unknown receiver must yield no completions: {items:?}"
    );
}
...
fn cmp06_inside_comment_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "comment interior must return empty completions"
    );
}
...
fn cmp07_filter_item_has_required_fields() {
    ...
    assert!(
        data.contains("truncate"),
        "data must contain the symbol name"
    );
}
...
fn cmp07_item_has_no_documentation_initially() {
    ...
    assert!(
        !any_with_doc,
        "items must not have documentation until resolved"
    );
}
...
fn cmp11_for_loop_var_offered_in_expression() {
    ...
    assert!(
        ...
        "for loop variable must be offered: {labels:?}"
    );
}
...
fn cmp12_path_context_offers_workspace_templates() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"base.html"),
        "workspace template must be offered: {labels:?}"
    );
}
...
fn cmp12_single_quoted_path_also_triggers_template_completion() {
    ...
    assert!(
        labels.contains(&"base.html"),
        "single-quoted extends must offer workspace templates: {labels:?}"
    );
}
...
fn cmp12_include_single_quoted_path_triggers_completion() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"nav.html"),
        "single-quoted include must offer workspace templates: {labels:?}"
    );
}
...
fn cmp08_local_macro_params_offered_in_call() {
    ...
    assert!(
        ...
        "title param must be offered: {labels:?}"
    );
    assert!(
        labels.iter().any(|l| l.starts_with("body")),
        "body param must be offered: {labels:?}"
    );
}
...
fn cmp08_params_offered_after_comma() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.iter().any(|l| l.starts_with("msg")),
        "msg param must be offered after comma: {labels:?}"
    );
}
...
fn cmp08_from_import_macro_params_offered() {
    ...
    ws.index_inline(
        "macros.html",
        "{% macro card(title, variant='default') %}{% endmacro %}",
    );
    let (items, _) = complete(src, 0, src.len() as u32, &idx, &reg(), &ws);
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.iter().any(|l| l.starts_with("title")),
        "title param must be offered for from-import: {labels:?}"
    );
}
...
fn cmp08_filter_call_kwargs_are_keyword_arg_kind() {
    ...
    assert!(
        !items.is_empty(),
        "must have items inside filter call paren"
    );
    for item in &items {
        assert_eq!(
            item.kind,
...
fn cmp04_from_import_offers_macro_names() {
    ...
    ws.index_inline(
        "macros.html",
        "{% macro post_url(post) %}{% endmacro %}{% macro card() %}{% endmacro %}",
    );
    let idx = extract(src);
    let (items, _) = complete(src, 0, src.len() as u32, &idx, &reg(), &ws);
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"post_url"),
        "post_url must be offered; got: {labels:?}"
    );
    assert!(
        labels.contains(&"card"),
        "card must be offered; got: {labels:?}"
    );
}
...
fn cmp04_from_import_after_comma_offers_remaining_macros() {
    ...
    assert!(
        labels.contains(&"card"),
        "card must be offered after comma; got: {labels:?}"
    );
}
...
fn cmp04_from_import_unknown_source_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "unknown source must return no completions; got: {items:?}"
    );
}
...
fn cmp09_end_inside_for_offers_only_endfor() {
    ...
    assert_eq!(
        labels,
        ["endfor"],
        "only endfor must be offered inside a for loop: {labels:?}"
    );
}
...
fn cmp09_end_inside_block_offers_only_endblock() {
    ...
    assert_eq!(
        labels,
        ["endblock"],
        "only endblock must be offered inside a block: {labels:?}"
    );
}
...
fn cmp09_end_with_no_open_block_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "no open block → no end-tag completions: {items:?}"
    );
}
...
fn cmp10_loop_offered_inside_for() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"loop"),
        "loop must be offered inside for: {labels:?}"
    );
}
...
fn cmp10_loop_not_offered_outside_for() {
    ...
    assert!(
        !labels.contains(&"loop"),
        "loop must not be offered outside for: {labels:?}"
    );
}
...
fn cmp12_folder_items_have_folder_kind() {
    ...
    let folder_labels: Vec<&str> = items
        .iter()
        .filter(|i| i.kind == CompletionKind::Folder)
        .map(|i| i.label.as_str())
        .collect();
    let file_labels: Vec<&str> = items
        .iter()
        .filter(|i| i.kind == CompletionKind::File)
        .map(|i| i.label.as_str())
        .collect();
    assert!(
        folder_labels.contains(&"blog/"),
        "blog/ must be offered as a folder: {folder_labels:?}"
    );
    assert!(
        file_labels.contains(&"base.html"),
        "base.html must be offered as a file: {file_labels:?}"
    );
    assert!(
        is_incomplete,
        "is_incomplete must be true when folders are present"
    );
}
...
fn cmp12_prefix_filters_to_correct_level() {
    ...
    assert!(
        ...
        "post.html must appear after typing blog/: {labels:?}"
    );
    assert!(
        !labels.contains(&"base.html"),
        "base.html must not appear after prefix blog/"
    );
}
...
fn fq3s_pipe_inside_string_literal_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "pipe inside string must not offer filters: {items:?}"
    );
}
...
fn fq3s_cursor_in_raw_block_body_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "cursor inside raw block must return empty: {items:?}"
    );
}
...
fn fq3s_cursor_in_raw_block_body_after_literal_tag_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "cursor inside raw block after a literal tag must return empty: {items:?}"
    );
}
...
fn fq3s_import_alias_slot_returns_empty() {
    ...
    assert!(
        items.is_empty(),
        "import alias slot must return empty: {items:?}"
    );
}
...
fn fq3s_from_import_alias_slot_returns_empty() {
    ...
    ws.index_inline(
        "macros.html",
        "{% macro card(title) %}{% endmacro %}{% macro header() %}{% endmacro %}",
    );
    let (items, _) = complete(src, 0, src.len() as u32, &idx, &reg(), &ws);
    assert!(
        items.is_empty(),
        "from-import alias slot must return empty: {items:?}"
    );
}
...
fn n6su_resolve_doc_handles_attr_prefix() {
    ...
    assert!(
        doc.is_some(),
        "resolve_doc must handle 'attr:parent.attr' data keys"
    );
}
...
fn cmp10_self_dot_offers_block_names_inside_block() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"content"),
        "self. must offer 'content' block: {labels:?}"
    );
    assert!(
        labels.contains(&"sidebar"),
        "self. must offer 'sidebar' block: {labels:?}"
    );
}
...
fn n6su_attr_item_data_key_is_resolvable() {
    ...
    for item in &items {
        if let Some(data) = &item.data {
            ...
            assert!(
                doc.is_some(),
                "attr item data '{data}' must resolve; got None"
            );
        }
    }
}
...
fn cmp12_absolute_key_workspace_offers_relative_template_paths() {
    ...
    let labels: Vec<&str> = items.iter().map(|i| i.label.as_str()).collect();
    assert!(
        labels.contains(&"blog/"),
        "blog/ must be offered as a folder: {labels:?}"
    );
    assert!(
        labels.contains(&"base.html"),
        "base.html must be offered as a file: {labels:?}"
    );
    assert!(
        !labels.iter().any(|l| l.starts_with('/')),
        "no bogus absolute-path entries: {labels:?}"
    );

    // Typing a real relative prefix must descend correctly.
    let src2 = r#"{% extends "blog/"#;
    let idx2 = extract(src2);
    let (items2, _) = complete(
        src2,
        0,
        src2.len() as u32,
        &idx2,
    ...
    assert!(
        labels2.contains(&"blog/post.html"),
        "blog/post.html must appear after typing 'blog/': {labels2:?}"
    );
